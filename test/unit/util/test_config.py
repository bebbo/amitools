...
def test_config_val_int_range():
    v = ConfigIntValue(int_range=(-10, 10))
    assert v.parse_value(-10) == -10
    assert v.parse_value(10) == 10
    with pytest.raises(ValueError):
        v.parse_value(-20)
...
def test_config_val_str_none():
    v = ConfigStringValue(allow_none=True)
    assert v.parse_value(None) is None
    assert v.parse_value("hello") is "hello"


def test_config_val_regex():
    v = ConfigRegExValue("a[bc].*d")
    assert v.parse_value("abd")
    assert v.parse_value("AbD")


def test_config_val_regex_case():
    v = ConfigRegExValue("a[bc].*d", case=True)
    assert v.parse_value("abcd")
    with pytest.raises(ValueError):
...
def test_config_args_dyn_value_ok_multi():
    s = create_config_set()
    c = ConfigCreator()
    pl = PreLogger()
    a = argparse.ArgumentParser()
    ...
    ap.add_dyn_value('grp', '-g', desc="set grp values")
    # parse arg list
    args = a.parse_args("-g bla=12 -g foo=b".split())
    ok = ap.parse(s, c, pl, args)
    assert ok
    assert pl.get_num_msgs(logging.WARNING) == 0
    assert pl.get_num_msgs(logging.ERROR) == 0
...
def test_config_args_dyn_group_ok_multi():
    s = create_config_set()
    c = ConfigCreator()
    pl = PreLogger()
    a = argparse.ArgumentParser()
    ...
    ap.add_dyn_group([gk], '-g', desc="set grp values")
    # parse arg list
    args = a.parse_args("-g grp:bla=12 -g grp:foo=b".split())
    ok = ap.parse(s, c, pl, args)
    assert ok
    assert pl.get_num_msgs(logging.WARNING) == 0
    assert pl.get_num_msgs(logging.ERROR) == 0
...
def test_config_main_parser_ok_cfg_name():
    s = create_config_set()
    d = {
        'grp': {
            'bla': 10
    ...
    c = ConfigMainParser(s, d, cfg_name="bluna", prog="foo")
    # pimp file cfg
    # g_cfg
    g_cfg = u"""
[grp]
